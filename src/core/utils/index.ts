 ...
 * 混合菜单
 * */
export function generatorMenuMix(routerMap: any[], routerName: string, location: string) {
 ...
 * 递归组装子菜单
 * */
export function getChildrenRouter(routerMap: any[]) {
    return filterRouter(routerMap).map((item) => {
        const isRoot = isRootRouter(item);
        const info = isRoot ? item.children[0] : item;
        const currentMenu = {
 ...
 * 排除Router
 * */
export function filterRouter(routerMap: any[]) {
